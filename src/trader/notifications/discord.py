...
class DiscordNotifier:
    ...
    async def _send(
        self, content: str | None = None, embed: dict | None = None
    ) -> bool:
    ...
    async def notify_trade_signal(
        ...
        if approved:
            color = (
                0x00FF00 if action == "BUY" else 0xFF6B6B
    ...
    async def notify_order_filled(
        ...
        embed = {
            ...
            "fields": [
                ...
                {
                    "name": "Quantity",
                    "value": str(order.filled_quantity),
                    "inline": True,
                },
                {
                    "name": "Fill Price",
                    "value": f"${float(filled_price):,.2f}",
                    "inline": True,
                },
                {"name": "Total Value", "value": f"${value:,.2f}", "inline": True},
                {
                    "name": "Order ID",
                    "value": order.broker_order_id or "N/A",
                    "inline": True,
                },
            ],
            "timestamp": datetime.utcnow().isoformat(),
            ...
            "fields": [
                {
                    ...
                    "value": f"${starting_equity:,.2f}",
                    "inline": True,
                },
                {
                    "name": "Ending Equity",
                    "value": f"${ending_equity:,.2f}",
                    "inline": True,
                },
                {
                    "name": "Daily Return",
                ...
                {
                    "name": "Win Rate",
                    "value": f"{win_rate:.0f}% ({winning_trades}W/{losing_trades}L)",
                    "inline": True,
                },
            ],
            "timestamp": datetime.utcnow().isoformat(),
    ...
    async def notify_position_update(
        ...
        if not positions:
            ...
            for pos in positions[:10]:  # Limit to 10
                ...
                lines.append(
                    f"**{pos.symbol}**: {pos.quantity} @ ${float(pos.avg_entry_price):,.2f} ({pnl_str})"
                )
            description = "\n".join(lines)
    ...
    async def notify_error(self, error: str, context: str | None = None) -> bool:
        ...
        if context:
            embed["fields"] = [
                {"name": "Context", "value": context[:1000], "inline": False}
            ]
